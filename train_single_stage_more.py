...
from utils import SAMPreprocess, PILToNumpy, NumpyToTensor, sample_points, is_valid_file
...
embed_model_type = 'clip' # clip, dino
guidance_method = 'concat' # concat, gsa
...
def custom_collate(batch):
    images, masks, embeddings = zip(*batch)
    return torch.stack(images), masks, torch.stack(embeddings)

train_loader = DataLoader(train_set, batch_size=batch_size, collate_fn=custom_collate, num_workers=num_workers, pin_memory=True)
val_loader = DataLoader(val_set, batch_size=batch_size, collate_fn=custom_collate, num_workers=num_workers, pin_memory=True)
test_loader = DataLoader(test_set, batch_size=batch_size, collate_fn=custom_collate, num_workers=num_workers, pin_memory=True)
...
def train_epoch(model, epoch, criterion, optimizer, dataloader, device):
    ...
    for batch, (image_batch, mask_batch, embedding_batch) in enumerate(tqdm.tqdm(dataloader)):
        # Transfer Data to GPU if available
        embedding_batch = embedding_batch.to(device)
        
        # masks is a list of num_batches mask tensors
        for image, masks, embedding in zip(image_batch, mask_batch, embedding_batch):
            images = image.repeat(len(masks), 1, 1, 1)
            embeddings = embedding.repeat(len(masks), 1, 1, 1)
            points = sample_points(masks).to(device)
            ...
            if example_method == 'ground_truth':
                ...
                # examples = torch.cat([examples, torch.rand((len(images), 10, example_dim), dtype=torch.float16).to(device)], 1)
            elif example_method == 'noisy_ground_truth':
                pass
                #examples = cutout_embeddings.unsqueeze(1).repeat(1, 5, 1)
